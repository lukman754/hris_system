...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
    <div class="p-4 rounded-lg shadow-sm relative overflow-hidden group" style="background: var(--surface);">
        <div data-theme-muted class="text-[8px] font-bold   opacity-40 mb-1">Total Workforce</div>
        <div data-theme-text class="text-2xl font-bold  leading-none"><?= $total_staff ?></div>
    ...
    <div class="p-4 rounded-lg shadow-sm relative overflow-hidden group" style="background: var(--surface);">
        <div data-theme-muted class="text-[8px] font-bold   opacity-40 mb-1">Active Today</div>
        <div class="text-2xl font-bold  leading-none text-emerald-500"><?= $total_present ?></div>
        <div class="absolute -right-2 -bottom-2 opacity-5 scale-150 rotate-12 group-hover:rotate-0 transition-all duration-500">
            <span class="material-symbols-outlined text-6xl text-emerald-500">bolt</span>
        </div>
    </div>
    <div class="p-4 rounded-lg shadow-sm relative overflow-hidden group" style="background: var(--surface);">
        <div data-theme-muted class="text-[8px] font-bold   opacity-40 mb-1">Departments</div>
        <div data-theme-text class="text-2xl font-bold  leading-none"><?= count($departments) ?></div>
        <div class="absolute -right-2 -bottom-2 opacity-5 scale-150 rotate-12 group-hover:rotate-0 transition-all duration-500">
            <span class="material-symbols-outlined text-6xl">account_tree</span>
        </div>
    </div>
    <div class="p-4 rounded-lg shadow-sm relative overflow-hidden group" style="background: var(--surface);">
        <div data-theme-muted class="text-[8px] font-bold   opacity-40 mb-1">Attendance Rate</div>
        <div data-theme-text class="text-2xl font-bold  leading-none"><?= $total_staff > 0 ? round(($total_present / $total_staff) * 100) : 0 ?>%</div>
        <div class="absolute -right-2 -bottom-2 opacity-5 scale-150 rotate-12 group-hover:rotate-0 transition-all duration-500">
...
<!-- People Grid -->
<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-4 pb-20">
    <?php foreach ($all_people as $p): 
        $wa = format_wa_link($p['phone_number']);
        $is_online = $p['is_present'] > 0;
    ?>
    <div class="person-card">
    <div data-theme-card class="bg-surface p-5 rounded-lg shadow-sm relative group hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-border h-full">
        <!-- Present Badge -->
        ...
        <div class="flex items-center justify-between pt-4">
            <div class="flex items-center gap-1">
                <div class="px-2 py-1 bg-surface-variant rounded text-[8px] font-bold   text-on-surface-variant/70 italic"><?= h($p['department']) ?></div>
            </div>
        ...
        <div class="mt-4 flex items-center gap-2 px-2 py-1.5 bg-emerald-500/[0.03] rounded-lg border border-emerald-500/5">
            ...
            <span class="text-[8px] font-bold text-emerald-600  ">Clocked in at <?= date('H:i', strtotime($p['check_in_time'])) ?></span>
        </div>
        <?php endif; ?>
    </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        cards.forEach(card => {
            // Find content inside h3 (name), p (position), and department div
            const name = card.querySelector('h3').textContent.toLowerCase();
            const position = card.querySelector('p').textContent.toLowerCase();
            const dept = card.querySelector('.italic').textContent.toLowerCase();
            
            if (name.includes(query) || position.includes(query) || dept.includes(query)) {
